...
class FormatConverter
{
    ...
    public static function formatToPrice($value, $currency = null)
    {
        ...
        $formattingInformation = localeconv();
        if ($currency && array_key_exists($currency, $currencyDecimals)) {
            if (strpos($value, $formattingInformation['decimal_point']) !== false && (floor($value) != $value)) {
        ...
        } elseif (strpos($value, $formattingInformation['decimal_point']) === false) {
