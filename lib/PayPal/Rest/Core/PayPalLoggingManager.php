...
namespace PayPal\Rest\Core;

use PayPal\Rest\Log\PayPalLogFactory;
...
class PayPalLoggingManager
{
    ...
    private function __construct($loggerName)
    {
        ...
        $factory = array_key_exists('log.AdapterFactory', $config) && in_array('PayPal\Rest\Log\PayPalLogFactory', class_implements($config['log.AdapterFactory'])) ? $config['log.AdapterFactory'] : '\PayPal\Rest\Log\PayPalDefaultLogFactory';
